...
static CONST uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = {WinyunqMouseIDName};
...
void HidEmu_Init( )
{
  ...
  uint16_t advInt = 3200;
  GAP_SetParamValue(TGAP_DISC_ADV_INT_MIN, 300);
  GAP_SetParamValue(TGAP_DISC_ADV_INT_MAX, 300);
...
uint16 HidEmu_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    MouseEvent();
    tmos_start_task( hidEmuTaskId, START_REPORT_EVT, MouseConfigure.details.report+7 );
