  ...
  { // 模拟传输4个字节的数据，实际传输根据用户需要自行修改
    if (Ready)
    {
      GPIOA_SetBits(LEDIndicator);
       MouseData.details.situation.left=!MouseData.details.situation.left;
       MouseData.details.x=1;
       MouseData.details.y=1;
      Ready = 0;
      DevHIDReport(MouseData.buffer,4);
    }

    mDelaymS(100);GPIOA_ResetBits(LEDIndicator);
    if(!Ready){mDelaymS(1000);Ready=1;}
  }
